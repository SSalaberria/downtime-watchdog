...
export class UserResolver {
  ...
  async updateUser(@ReqUser() user: Payload, @Args('updateUserInput') updateUserInput: UpdateUserInput): Promise<User> {
    return this.users.update(new Types.ObjectId(user.id), updateUserInput);
  }

  @Query(() => User)
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles(Role.ADMIN)
  async findById(@Args('_id', { type: () => String }) id: Types.ObjectId): Promise<User> {
  ...
  async user(@ReqUser() user: Payload): Promise<User> {
    return this.users.findOneById(new Types.ObjectId(user.id));
  }

  @ResolveField()
  async dashboard(@Parent() user: UserDocument): Promise<User> {
    await user.populate({ path: 'dashboard', model: 'Dashboard' });
